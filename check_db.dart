...
void main() async {
  ...
  final productsDist = await db.rawQuery('SELECT store_id, COUNT(*) as count FROM local_products GROUP BY store_id');
  for (var row in productsDist) {
    print('Store ID: ${row['store_id']} | Count: ${row['count']}');
  }
  
  print('\n--- Business Partners Store Distribution ---');
  final partnersDist = await db.rawQuery('SELECT store_id, COUNT(*) as count FROM local_businesspartners GROUP BY store_id');
