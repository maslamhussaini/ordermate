...
class InventoryGeneralJournalReportScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<InventoryGeneralJournalReportScreen> createState() => _InventoryGeneralJournalReportScreenState();
}

class _InventoryGeneralJournalReportScreenState extends ConsumerState<InventoryGeneralJournalReportScreen> {
  ...
  void initState() {
    ...
    Future.microtask(() {
       ...
       ref.read(organizationProvider.notifier).loadOrganizations(); // Loads stores
       _generateReport();
    });
  }
  ...
  Future<void> _generateReport() async {
    ...
    for (final t in transfers) {
      if (t.transferDate.isBefore(_startDate) || t.transferDate.isAfter(_endDate.add(const Duration(days: 1)))) continue;
      ...
      for (final item in t.items) {
        if (_selectedProductId != null && item.productId != _selectedProductId) continue;
        ...
        // In to Destination
        if (t.destinationStoreId != null && (_selectedStoreId == null || t.destinationStoreId == _selectedStoreId)) {
           entries.add(JournalEntry(
             date: t.transferDate,
             type: 'Transfer In',
             reference: t.transferNumber,
    ...
    for (final o in orders) {
       // Filter Date
       if (o.orderDate.isBefore(_startDate) || o.orderDate.isAfter(_endDate.add(const Duration(days: 1)))) continue;
       ...
       for (final line in o.items) {
          if (_selectedProductId != null && line.productId != _selectedProductId) continue;
      ...
      body: Column(
        children: [
                     ...
                     children: [
                       Expanded(
                         child: DropdownButtonFormField<int>(
                           ...
                           decoration: const InputDecoration(labelText: 'Store', isDense: true, border: OutlineInputBorder()),
                           items: [
          ...
          Expanded(
            ...
            : _reportData.isEmpty 
              ? const Center(child: Text('No records found for criteria.'))
              : SingleChildScrollView(
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: DataTable(
                      ...
                      rows: _reportData.map((e) => DataRow(cells: [
                        DataCell(Text(DateFormat('yy-MM-dd').format(e.date))),
                        DataCell(Text(e.storeName)),
                        DataCell(Text(e.productName)),
                        DataCell(Text(e.reference)),
                        DataCell(Text(e.type)),
                        DataCell(Text(e.qtyIn > 0 ? e.qtyIn.toStringAsFixed(2) : '-', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.green))),
                        DataCell(Text(e.qtyOut > 0 ? e.qtyOut.toStringAsFixed(2) : '-', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.red))),
                      ])).toList(),
                    ),
                  ),
                ),
