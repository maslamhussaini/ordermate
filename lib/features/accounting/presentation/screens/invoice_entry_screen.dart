...
class _InvoiceEntryScreenState extends ConsumerState<InvoiceEntryScreen> {
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _invoiceItems = items.map((i) => {
            'product_id': i['product_id'],
            'product_name': i['product_name'] ?? 'Product',
            'quantity': i['quantity'] is int ? (i['quantity'] as int).toDouble() : i['quantity'] as double,
            'uom_id': i['uom_id'],
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error loading from order: $e')));
        setState(() => _isLoading = false);
      }
    }
  }
  ...
  Future<void> _loadExistingInvoice() async {
    ...
    try {
      ...
      final invoice = invoices.firstWhere((i) => i.id == widget.invoiceId, orElse: () {
         throw Exception('Invoice not found');
      });
      
      final items = await ref.read(accountingProvider.notifier).getInvoiceItems(widget.invoiceId!);
      ...
      if (mounted) {
        setState(() {
          ...
          final partner = ref.read(businessPartnerProvider).customers
          ...
          if (invoice.sYear != null) {
             ...
             final session = state.financialSessions.cast<FinancialSession?>().firstWhere(
                 (s) => s?.sYear == invoice.sYear, 
                 orElse: () => null
             );
             if (session != null && session.isClosed) {
                _isClosedYear = true;
                _readOnlyReason = 'This invoice belongs to a closed financial year (${session.sYear}).';
             }
          }
  ...
  double get _calculateTotalAmount {
    return _invoiceItems.fold(0, (sum, item) => sum + (item['total'] as double));
  }
      ...
      final item = _invoiceItems[index]; 
       setState(() {
          ...
          try {
             ...
             _selectedProduct = productList.firstWhere((p) => p.id == item['product_id']);
             _productSearchController.text = _selectedProduct!.name;
          } catch (_) {
             ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Warning: Product details not fully loaded.')));
          }
  ...
  void _addItem() {
    ...
    final minRate = (_selectedProduct?.limitPrice ?? 0) * (_selectedUomFactor / productBaseQty);
    if (rate < minRate) {
    ...
    if (limit > 0) {
       ...
       if (discount > effectiveLimit) {
         ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('Discount cannot exceed the limit of ${effectiveLimit.toStringAsFixed(2)}%'),
          backgroundColor: Colors.red,
        ));
        return;
       }
  ...
  Future<void> _submit() async {
    ...
    final message = ValueNotifier<String>(isEdit ? 'Updating Invoice...' : 'Posting to Transactions...');
    ...
    final success = await showDialog<bool>(
      ...
      builder: (context) => ProcessingDialog(
        ...
        successMessage: isEdit ? 'Updated Successfully!' : 'Posted Successfully!',
        progressNotifier: progress,
        messageNotifier: message,
        task: () async {
            progress.value = 0.1;
            message.value = 'Validating Customer...';
            await Future.delayed(const Duration(milliseconds: 300)); // Dramatic effect
            ...
            final invoice = Invoice(
              ...
              sYear: ref.read(accountingProvider).financialSessions.cast<FinancialSession?>().firstWhere(
                  (s) => s != null && 
                         (_invoiceDate.isAtSameMomentAs(s.startDate) || _invoiceDate.isAfter(s.startDate)) &&
                         (_invoiceDate.isAtSameMomentAs(s.endDate) || _invoiceDate.isBefore(s.endDate.add(const Duration(days: 1)))), 
                  orElse: () => null,
              )?.sYear,
            );
            
            print('DEBUG: Submitting Invoice. Date: $_invoiceDate, Resolved sYear: ${invoice.sYear}');
            
            progress.value = 0.5;
            message.value = isEdit ? 'Saving Changes...' : 'Saving Invoice & Posting to GL...';

            if (isEdit) {
              await ref.read(accountingProvider.notifier).updateInvoiceWithItems(invoice, _invoiceItems);
            } else {
              await ref.read(accountingProvider.notifier).createInvoiceWithItems(invoice, _invoiceItems);
            }
  ...
  Widget build(BuildContext context) {
    ...
    final currencySymbol = orgState.selectedStore?.storeDefaultCurrency ?? 'AED';
                        ...
                        Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(child: Text(_readOnlyReason!, style: const TextStyle(color: Colors.black87))),
                            ],
                          ),
                        ),
                      // Header Card
                          ...
                          children: [
                            ...
                            Autocomplete<BusinessPartner>(
                              ...
                              optionsBuilder: (TextEditingValue textEditingValue) {
                                if (textEditingValue.text.isEmpty) return const Iterable<BusinessPartner>.empty();
                                return partnerState.customers.where((BusinessPartner option) {
                                  return option.name.toLowerCase().contains(textEditingValue.text.toLowerCase());
                                });
                              },
                              onSelected: (BusinessPartner selection) => setState(() => _selectedPartner = selection),
                          ...
                          children: [
                            ...
                            Autocomplete<Product>(
                              ...
                              optionsBuilder: (TextEditingValue textEditingValue) {
                                ...
                                return productState.products.where((Product option) {
                                  return option.name.toLowerCase().contains(textEditingValue.text.toLowerCase());
                                });
                              },
                                ...
                                Container(
                                   ...
                                   child: Column(
                                     ...
                                     children: [
                                       ...
                                       Text('Price: ${currencyFormat.format(_selectedProduct!.rate)} / ${_selectedProduct!.baseQuantity} ${_selectedProduct!.uomSymbol ?? 'Unit'}'),
                                     ],
                                   ),
                                ),
                                const SizedBox(height: 12),
                             ...
                             if (_invoiceItems.isEmpty)
                               ...
                               SingleChildScrollView(
                                 ...
                                 child: DataTable(
                                   ...
                                   rows: List.generate(_invoiceItems.length, (index) {
                                     final item = _invoiceItems[index];
                                     return DataRow(
                                         ...
                                         DataCell(Row(
                                           ...
                                           children: [
                                             IconButton(
                                               ...
                                               onPressed: _isReadOnly ? null : () => _editItemAt(index),
                                             ),
                                             IconButton(
                                               icon: const Icon(Icons.delete, color: Colors.red), 
                                               onPressed: _isReadOnly ? null : () => _removeItem(index),
                                             ),
                                           ],
                                         )),
                                       ],
