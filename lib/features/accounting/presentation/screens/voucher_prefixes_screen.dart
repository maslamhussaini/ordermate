...
class VoucherPrefixesScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<VoucherPrefixesScreen> createState() => _VoucherPrefixesScreenState();
}
...
class _VoucherPrefixesScreenState extends ConsumerState<VoucherPrefixesScreen> {
  ...
  Future<void> _downloadTemplate() async {
    try {
      final headers = [
        ['Prefix Code', 'Description', 'Voucher Type', 'Status (1=Active, 0=Inactive)'],
      ];
      final path = await CsvService().saveCsvFile('voucher_prefix_template.csv', headers);
      ...
      await Future(() async {
        ...
        for (var i = startIndex; i < rows.length; i++) {
          ...
          try {
             ...
             final voucherType = row.length > 2 ? row[2].toString().trim() : 'General';
             final statusVal = row.length > 3 ? int.tryParse(row[3].toString()) : 1;
             ...
             await ref.read(accountingProvider.notifier).addVoucherPrefix(newPrefix);
        ...
        if (mounted && !isCancelled) {
             ...
             if (mounted) {
               ...
               ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text('Import Complete: $successCount, Duplicates: $duplicateCount, Failed: $failCount')),
               );
                          ...
                          return Card(
                            child: ListTile(
                              onTap: () {
                                if (prefix.isSystem) {
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(content: Text('System defined prefixes cannot be edited.')),
                                  );
                                } else {
                                  context.push('/accounting/voucher-prefixes/edit/${prefix.id}');
                                }
                              },
                              leading: CircleAvatar(
                                ...
                                child: Icon(
                                  ...
                                  color: prefix.isSystem ? Colors.grey : Colors.purple,
                                  size: 20,
                                ),
                              ),
                              title: Row(
                                children: [
                                  Text(prefix.prefixCode, style: const TextStyle(fontWeight: FontWeight.bold)),
                                  if (prefix.isSystem)
                                    Container(
                                      ...
                                      child: const Text('SYSTEM', style: TextStyle(fontSize: 10, color: Colors.blue)),
                                    ),
                                ],
                              ),
                              subtitle: prefix.description != null ? Text(prefix.description!) : null,
                              trailing: Row(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  Icon(
                                    prefix.status ? Icons.check_circle : Icons.cancel,
                                    color: prefix.status ? Colors.green : Colors.red,
                                  ),
                                  if (!prefix.isSystem)
                                    IconButton(
  ...
  Future<void> _confirmDelete(VoucherPrefix prefix) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text('Are you sure you want to delete "${prefix.prefixCode}"?'),
        actions: [
    ...
    if (confirmed == true && mounted) {
      try {
        final orgId = ref.read(organizationProvider).selectedOrganization?.id;
        await ref.read(accountingProvider.notifier).deleteVoucherPrefix(prefix.id, organizationId: orgId);
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Voucher prefix deleted successfully')),
          );
        }
      } catch (e) {
        if (mounted) {
