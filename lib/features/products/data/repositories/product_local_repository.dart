...
class ProductLocalRepository {
  ...
  Future<List<Product>> getLocalProducts({int? organizationId, int? storeId}) async {
    final db = await _dbHelper.database;
  ...
  Future<List<Product>> getUnsyncedProducts({int? organizationId, int? storeId}) async {
    final db = await _dbHelper.database;
    String sql = '''
      SELECT 
        p.*,
  ...
  Future<void> markProductAsSynced(String id) async {
    final db = await _dbHelper.database;
    await db.update('local_products', {'is_synced': 1}, where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<void> updateProduct(Product p) async {
    final db = await _dbHelper.database;
    await db.update('local_products', {
         'name': p.name,
         'sku': p.sku,
         'rate': p.rate,
         'cost': p.cost,
  ...
  Product _mapToProduct(Map<String, dynamic> map) {
      return Product(
        ...
        limitPrice: (map['limit_price'] as num? ?? map['limtprice'] as num?)?.toDouble() ?? 0.0,
        ...
        revenueGlId: (map['revenue_gl_id'] as String?) ?? (map['revnue_id'] as String?),
        defaultDiscountPercent: (map['defult_discount_percnt'] as num?)?.toDouble() ?? 0.0,
        defaultDiscountPercentLimit: (map['defult_discount_percnt_limit'] as num?)?.toDouble() ?? 0.0,
        salesDiscountGlId: (map['sales_discount_id'] as String?) ?? (map['sales_discount_gl_id'] as String?),
      );
  }
}
