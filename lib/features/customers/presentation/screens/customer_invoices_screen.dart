...
class CustomerInvoicesScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CustomerInvoicesScreen> createState() => _CustomerInvoicesScreenState();
}

class _CustomerInvoicesScreenState extends ConsumerState<CustomerInvoicesScreen> {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Column(
          ...
          children: [
            const Text('Invoices List', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            Text(widget.customer.name, style: TextStyle(fontSize: 13, color: Colors.indigo.shade300)),
          ],
        ),
          ...
          ? const Center(
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 16),
                  Text('Fetching invoices...', style: TextStyle(color: Colors.grey)),
                ],
              ),
            )
          : _errorMessage != null
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('Error loading invoices', style: Theme.of(context).textTheme.titleMedium),
                        const SizedBox(height: 8),
                        Text(_errorMessage!, textAlign: TextAlign.center, style: const TextStyle(color: Colors.grey)),
                        const SizedBox(height: 24),
                        ElevatedButton(onPressed: _loadInvoices, child: const Text('Try Again')),
                      ],
                    ),
                  ),
                )
                  ...
                  ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          const SizedBox(height: 16),
                          const Text('No pending invoices found.', style: TextStyle(color: Colors.grey, fontSize: 16)),
                          const SizedBox(height: 8),
                          const Text('All invoices for this customer are paid or none exist.', style: TextStyle(color: Colors.grey, fontSize: 12)),
                        ],
                      ),
                    )
                  : RefreshIndicator(
                              ...
                              child: Container(
                                ...
                                child: DataTable(
                                  ...
                                  rows: _invoices.map((inv) {
                                    final invoiceDate = DateTime.fromMillisecondsSinceEpoch(inv['invoice_date']);
                                    final dueDate = inv['due_date'] != null ? DateTime.fromMillisecondsSinceEpoch(inv['due_date']) : null;
                                    
                                    final now = DateTime.now();
                                    int agingDays = 0;
                                    if (dueDate != null) {
                                      agingDays = now.difference(dueDate).inDays;
                                    }
                                    ...
                                    return DataRow(cells: [
                                      DataCell(Text(inv['invoice_number'] ?? '', style: const TextStyle(fontWeight: FontWeight.w500))),
                                      DataCell(Text(DateFormat('MMM dd, yyyy').format(invoiceDate))),
                                      DataCell(Text('$_currency ${totalAmount.toStringAsFixed(2)}')),
                                      DataCell(Text('$_currency ${paidAmount.toStringAsFixed(2)}', style: const TextStyle(color: Colors.grey))),
                                      DataCell(Text('$_currency ${balance.toStringAsFixed(2)}', style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.green))),
                                      DataCell(
                                        Container(
                                          padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                                          decoration: BoxDecoration(
                                            color: agingDays > 0 ? Colors.red.shade50 : Colors.blue.shade50,
                                            borderRadius: BorderRadius.circular(4),
                                          ),
                                          child: Text(
                                            agingDays > 0 ? "$agingDays d late" : agingDays == 0 ? "Due today" : "${agingDays.abs()} d left",
                                            style: TextStyle(
                                              color: agingDays > 0 ? Colors.red : Colors.blue,
                                              fontWeight: FontWeight.bold,
                                              fontSize: 11,
                                            ),
                                          ),
                                      ...
                                      DataCell(
                                        ElevatedButton(
                                          onPressed: () => _showReceiptDialog(widget.customer, inv),
  ...
  void _showReceiptDialog(BusinessPartner customer, Map<String, dynamic> invoice) async {
    final accountingRepo = ref.read(accountingRepositoryProvider);
    final notifier = ref.read(accountingProvider.notifier);
    
    // Check GL Setup
    final glSetup = await accountingRepo.getGLSetup(customer.organizationId ?? 0);
    
    if (glSetup == null) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('GL Setup not found. Please configure accounting settings.')));
      return;
    }
    ...
    final narrationController = TextEditingController(text: 'Payment for Invoice ${invoice['invoice_number']}');

    showDialog(
      context: context,
      builder: (context) => StatefulBuilder(
        builder: (context, setDialogState) => AlertDialog(
          title: Text('Receipt - ${customer.name}', style: const TextStyle(color: Colors.indigo)),
          content: SingleChildScrollView(
            child: Column(
              mainAxisSize: MainAxisSize.min,
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('Invoice #: ${invoice['invoice_number']}', style: const TextStyle(fontWeight: FontWeight.bold)),
                const SizedBox(height: 16),
                const Text('Payment Type', style: TextStyle(fontWeight: FontWeight.w500, fontSize: 13)),
                const SizedBox(height: 8),
                Row(
                  children: [
                    Expanded(
                      child: ChoiceChip(
                        ...
                        onSelected: (val) => setDialogState(() => paymentType = 'Cash'),
                    ...
                    Expanded(
                      child: ChoiceChip(
                        ...
                        onSelected: (val) => setDialogState(() => paymentType = 'Bank'),
                        selectedColor: Colors.indigo.shade100,
                      ),
                    ),
                  ],
                ...
                TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    helperText: 'Balance: $_currency ${balance.toStringAsFixed(2)}',
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                final targetAccountId = paymentType == 'Cash' ? glSetup.cashAccountId : glSetup.bankAccountId;
                
                if (targetAccountId == null || targetAccountId.isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: No ${paymentType.toLowerCase()} account configured in GL Setup.')));
                  return;
                }

                if (customer.chartOfAccountId == null || customer.chartOfAccountId!.isEmpty) {
                   ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Error: Customer does not have a linked Chart of Account.')));
                   return;
                }

                final targetPrefixCode = paymentType == 'Cash' ? 'CRV' : 'BRV';
                final prefix = prefixes.where((p) => p.prefixCode == targetPrefixCode).firstOrNull;

                if (prefix == null) {
                  ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error: No voucher prefix found for $targetPrefixCode.')));
                  return;
                }
                ...
                if (enteredAmount > (balance + 0.01)) { // Allow slight rounding diff
                   ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Amount cannot exceed the balance criteria ($_currency ${balance.toStringAsFixed(2)}).')));
                   return;
                }
                ...
                try {
                  ...
                  final updatedInvoice = Invoice(
                    ...
                    invoiceDate: DateTime.fromMillisecondsSinceEpoch(invoice['invoice_date']),
                    dueDate: invoice['due_date'] != null ? DateTime.fromMillisecondsSinceEpoch(invoice['due_date']) : null,
                  ...
                  if (context.mounted) {
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Receipt saved. Invoice status: $status')));
                    _loadInvoices(); // Refresh list
                  }
                } catch (e) {
                  if (context.mounted) {
                    ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error saving receipt: $e')));
                  }
                }
              },
