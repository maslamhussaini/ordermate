...
class InventoryLocalRepository {
  ...
  Future<void> cacheBrands(List<Brand> brands) async {
    ...
    final Set<int> unsyncedIds = unsyncedMaps.map((m) => m['id'] as int).toSet();

    for (var item in brands) {
       if (unsyncedIds.contains(item.id)) continue;

       batch.insert('local_brands', {
         'id': item.id,
         'name': item.name,
         'status': item.status,
  ...
  Future<List<Brand>> getLocalBrands({int? organizationId}) async {
    ...
    final maps = await db.query('local_brands', where: where, whereArgs: args, orderBy: 'name ASC');
  ...
  Future<int> saveBrand(Brand item, {bool isSynced = false}) async {
    ...
    final id = await db.insert('local_brands', map, conflictAlgorithm: ConflictAlgorithm.replace);
    return id;
  }
  ...
  Future<void> markBrandAsSynced(int id) async {
    final db = await _dbHelper.database;
    await db.update('local_brands', {'is_synced': 1}, where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<List<ProductCategory>> getLocalCategories({int? organizationId}) async {
    final db = await _dbHelper.database;
    String where = 'status = 1';
    List<dynamic> args = [];
    ...
    final maps = await db.query('local_categories', where: where, whereArgs: args, orderBy: 'name ASC');
  ...
  Future<int> saveCategory(ProductCategory item, {bool isSynced = false}) async {
    final db = await _dbHelper.database;
    final map = {
      'name': item.name,
      'status': item.status,
    ...
    final id = await db.insert('local_categories', map, conflictAlgorithm: ConflictAlgorithm.replace);
    return id;
  }

  Future<List<ProductCategory>> getUnsyncedCategories({int? organizationId}) async {
    final db = await _dbHelper.database;
    String where = 'is_synced = 0';
    List<dynamic> args = [];
    ...
    final maps = await db.query('local_categories', where: where, whereArgs: args);
  ...
  Future<void> markCategoryAsSynced(int id) async {
    final db = await _dbHelper.database;
    await db.update('local_categories', {'is_synced': 1}, where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<void> cacheProductTypes(List<ProductType> items) async {
    ...
    for (var item in items) {
       ...
       batch.insert('local_product_types', {
         'id': item.id,
         'name': item.name,
         'status': item.status,
         'organization_id': item.organizationId,
  ...
  Future<List<ProductType>> getLocalProductTypes({int? organizationId}) async {
    ...
    final maps = await db.query('local_product_types', where: where, whereArgs: args, orderBy: 'name ASC');
  ...
  Future<int> saveProductType(ProductType item, {bool isSynced = false}) async {
    ...
    final id = await db.insert('local_product_types', map, conflictAlgorithm: ConflictAlgorithm.replace);
    return id;
  }

  Future<List<ProductType>> getUnsyncedProductTypes({int? organizationId}) async {
    final db = await _dbHelper.database;
    String where = 'is_synced = 0';
    List<dynamic> args = [];
    ...
    final maps = await db.query('local_product_types', where: where, whereArgs: args);
    return maps.map((map) => ProductType(
      id: map['id'] as int,
      name: map['name'] as String,
      status: map['status'] as int,
      organizationId: (map['organization_id'] as int?) ?? 0,
  ...
  Future<void> markProductTypeAsSynced(int id) async {
    final db = await _dbHelper.database;
    await db.update('local_product_types', {'is_synced': 1}, where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<void> cacheUnitsOfMeasure(List<UnitOfMeasure> items) async {
    ...
    for (var item in items) {
       ...
       batch.insert('local_units_of_measure', {
         'id': item.id,
         'unit_name': item.name,
  ...
  Future<List<UnitOfMeasure>> getLocalUnitsOfMeasure({int? organizationId}) async {
    final db = await _dbHelper.database;
    String where = 'status = 1';
    List<dynamic> args = [];
    ...
    final maps = await db.query('local_units_of_measure', where: where, whereArgs: args, orderBy: 'unit_name ASC');
  ...
  Future<int> saveUnitOfMeasure(UnitOfMeasure item, {bool isSynced = false}) async {
    final db = await _dbHelper.database;
    final map = {
      'unit_name': item.name,
      'unit_symbol': item.symbol,
    ...
    final id = await db.insert('local_units_of_measure', map, conflictAlgorithm: ConflictAlgorithm.replace);
    return id;
  }

  Future<List<UnitOfMeasure>> getUnsyncedUnitsOfMeasure({int? organizationId}) async {
    final db = await _dbHelper.database;
    String where = 'is_synced = 0';
    List<dynamic> args = [];
    ...
    final maps = await db.query('local_units_of_measure', where: where, whereArgs: args);
    return maps.map((map) => UnitOfMeasure(
      id: map['id'] as int,
      name: map['unit_name'] as String,
      symbol: map['unit_symbol'] as String,
      type: map['unit_type'] as String?,
  ...
  Future<void> markUnitOfMeasureAsSynced(int id) async {
    final db = await _dbHelper.database;
    await db.update('local_units_of_measure', {'is_synced': 1}, where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<void> cacheUnitConversions(List<UnitConversion> items) async {
    ...
    for (var item in items) {
       ...
       batch.insert('local_unit_conversions', {
         'id': item.id,
  ...
  Future<List<UnitConversion>> getLocalUnitConversions({int? organizationId}) async {
    final db = await _dbHelper.database;
    String where = 'status = 1';
    List<dynamic> args = [];
    ...
    final maps = await db.query('local_unit_conversions', where: where, whereArgs: args);
  ...
  Future<int> saveUnitConversion(UnitConversion item, {bool isSynced = false}) async {
    final db = await _dbHelper.database;
    final map = {
      'from_unit_id': item.fromUnitId,
      'to_unit_id': item.toUnitId,
    ...
    final id = await db.insert('local_unit_conversions', map, conflictAlgorithm: ConflictAlgorithm.replace);
    return id;
  }

  Future<List<UnitConversion>> getUnsyncedUnitConversions({int? organizationId}) async {
    final db = await _dbHelper.database;
    String where = 'is_synced = 0';
    List<dynamic> args = [];
    ...
    final maps = await db.query('local_unit_conversions', where: where, whereArgs: args);
    return maps.map((map) => UnitConversion(
      id: map['id'] as int,
      fromUnitId: map['from_unit_id'] as int,
      toUnitId: map['to_unit_id'] as int,
      conversionFactor: (map['conversion_factor'] as num).toDouble(),
  ...
  Future<void> markUnitConversionAsSynced(int id) async {
    final db = await _dbHelper.database;
    await db.update('local_unit_conversions', {'is_synced': 1}, where: 'id = ?', whereArgs: [id]);
  }
}
