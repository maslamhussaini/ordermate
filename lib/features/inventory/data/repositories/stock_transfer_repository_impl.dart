...
class StockTransferRepositoryImpl implements StockTransferRepository {
  ...
  Future<List<StockTransfer>> getTransfers({int? organizationId, int? storeId}) async {
    ...
    try {
      ...
      return await _localRepository.getTransfers(organizationId: organizationId, storeId: storeId);
  ...
  Future<StockTransfer> createTransfer(StockTransfer transfer) async {
    ...
    try {
      ...
      json.remove('created_at'); // Let DB specific handling if needed, or send it (usually preferred to control)
      ...
      if (transfer.items.isNotEmpty) {
        ...
        await SupabaseConfig.client.from('omtbl_stock_transfer_items').upsert(itemsJson);
      }
      ...
      final unsyncedModel = StockTransferModel.fromEntity(transfer.copyWith(isSynced: false));
  ...
  Future<void> updateTransfer(StockTransfer transfer) async {
      ...
      try {
        ...
        await SupabaseConfig.client.from('omtbl_stock_transfer_items').delete().eq('transfer_id', transfer.id);
  ...
  Future<void> deleteTransfer(String id) async {
      ...
      try {
        ...
        await SupabaseConfig.client.from('omtbl_stock_transfer_items').delete().eq('transfer_id', id);
